...
Parser.prototype.generateBit = function(ctx) {
  ...
  if (
    ...
    if (sum <= 8) {
      ...
      var val1 = ctx.generateTmpVariable();
      var val2 = ctx.generateTmpVariable();
      ctx.pushCode("var {0} = buffer.readUInt32BE(offset);", val1);
      ctx.pushCode("var {0} = buffer.readUInt8(offset + 4);", val2);
      ctx.pushCode("var {2} = ({0} * 256) + {1};", val1, val2, val);
      sum = 40;
    } else if (sum <= 48) {
      var val1 = ctx.generateTmpVariable();
      var val2 = ctx.generateTmpVariable();
      ctx.pushCode("var {0} = buffer.readUInt32BE(offset);", val1);
      ctx.pushCode("var {0} = buffer.readUInt16BE(offset + 4);", val2);
      ctx.pushCode("var {2} = ({0} * 65536) + {1};", val1, val2, val);
      sum = 48;
    } else {
      throw new Error(
        "Currently, bit field sequence longer than 6-bytes is not supported."
      );
    }
    ...
    ctx.bitFields.forEach(function(parser) {
      var tmpvar = ctx.generateTmpVariable();
      ctx.pushCode(
        "{0} = Math.floor({1} / Math.pow(2,{2}));",
        tmpvar,
        val,
      ...
      ctx.pushCode(
        "{0} = {1} - (Math.pow(2,{2}) * Math.floor({1} / Math.pow(2,{2})));",
        parser.varName,
        tmpvar,
        parser.options.length
      );
